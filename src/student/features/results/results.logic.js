...
export const ResultsManager = {
  async loadResults() {
    ...
    if (cached) {
      ...
      contentEl.innerHTML = '<div class="p-10 text-center text-gray-400"><i class="fas fa-wifi-slash text-4xl mb-3 opacity-30"></i><p>অফলাইনে ফলাফল পাওয়া যায়নি</p></div>';
    }
  },

  _renderResultsList(contentEl, resultsData, subjectList) {
  ...
  async viewDetailedResult(attemptId) {
    ...
    const cacheKey = 'detailResult_' + attemptId;
    ...
    if (navigator.onLine) {
      try {
        ...
        const detail = { att, exam, questions };
        await cacheData(cacheKey, detail);
        this._renderDetailedResult(contentEl, detail);
        return;
      } catch (error) {
    ...
    const cached = await getCachedData(cacheKey);
    if (cached) {
      this._renderDetailedResult(contentEl, cached);
    } else {
      contentEl.innerHTML = '<div class="p-10 text-center text-gray-400">অফলাইনে বিস্তারিত ফলাফল পাওয়া যায়নি</div>';
    }
  },

  _renderDetailedResult(contentEl, detail) {
    const { att, exam, questions: qs } = detail;
    currentResultPage = 1;
    resultFilter = 'all';
