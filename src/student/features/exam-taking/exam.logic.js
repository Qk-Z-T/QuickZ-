  ...
  async render() {
      ...
      qHTML += `
        <div class="p-4 rounded-xl shadow-sm border mb-4 bg-white dark:bg-gray-800" id="q-${i}">
          <div class="flex justify-between items-center mb-3">
            ...
            <button id="mark-btn-${i}" onclick="Exam.toggleMark(${i})" class="text-xs font-bold ${this.marked[i] ? 'text-amber-500' : 'text-gray-400 dark:text-gray-500'}">
          ...
          <div class="space-y-2">
            ${q.options.map((o, oi) => {
              ...
              return `
                <button onclick="Exam.sel(${i},${oi})"
                  class="opt-btn option-btn w-full text-left p-3 rounded-lg border text-sm flex gap-2 transition ${selected} ${locked}"
                  ${isAnswered ? 'disabled' : ''}
                  id="opt-${i}-${oi}">
                  <span class="font-bold opacity-50 w-6">${String.fromCharCode(65+oi)}.</span>
                  <span class="flex-1 text-left bengali-text">${optText}</span>
                  ${selected ? '<i class="fas fa-check-circle text-green-600 ml-2 text-lg"></i>' : ''}
                </button>`;
            }).join('')}
          </div>
        </div>`;
    ...
    const paginationHTML = totalPages > 1 ? `
      <div class="flex justify-center gap-2 mt-6">
        ...
        <span class="px-4 py-2 text-sm dark:text-white">পৃষ্ঠা ${this.currentPage + 1}/${totalPages}</span>
    ...
    <div class="sticky top-0 border-b px-4 py-3 flex justify-between items-center z-30 shadow-md bg-white/95 dark:bg-gray-900/95">
      <div>
        ...
        <div class="text-center">
          <span class="font-bold block text-sm truncate w-32 mx-auto dark:text-white">${this.d.title}</span>
          <div class="flex items-center justify-center gap-2 mt-1">
            <span id="tm" class="text-xs font-mono bg-gray-100 dark:bg-gray-800 px-1 rounded dark:text-white">00:00</span>
            <span id="answered-counter" class="text-xs font-mono bg-indigo-100 dark:bg-indigo-900 text-indigo-600 dark:text-indigo-400 px-1 rounded">সম্পন্ন: ${answeredCount}/${this.d.qs.length}</span>
          </div>
        </div>
      </div>
