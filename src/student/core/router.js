...
// Student portal routing and page rendering
...
// Lazy imports for features (to avoid circular deps)
async function getStudentModule() {
  const { Student } = await import('../features/dashboard/dashboard.logic.js');
  return Student;
}
...
function renderHeader(activePage) {
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-4">
        ...
        <div class="relative course-switcher-dropdown">
          <button id="course-switcher-btn" onclick="Student.toggleCourseSwitcher()" class="flex items-center gap-2 px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm font-bold transition">
          ...
          <div id="course-switcher-menu" class="hidden absolute top-full left-0 mt-2 w-64 rounded-xl shadow-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 z-50 max-h-80 overflow-y-auto">
            <div id="course-switcher-list" class="py-2">
              ${AppState.joinedGroups && AppState.joinedGroups.length > 0 ?
                AppState.joinedGroups.map(g => `
                  <div class="px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition flex items-center justify-between ${g.groupId === AppState.activeGroupId ? 'bg-indigo-50 dark:bg-indigo-900/30' : ''}" onclick="Student.switchCourseFromDropdown('${g.groupId}')">
            ...
            <div class="border-t border-gray-200 dark:border-gray-600 p-2">
              <button onclick="Router.student('courses'); Student.toggleCourseSwitcher();" class="w-full text-center text-xs text-indigo-600 dark:text-indigo-400 font-bold py-2 hover:underline"><i class="fas fa-plus mr-1"></i> নতুন কোর্স খুঁজুন</button>
            </div>
          </div>
        </div>
      </div>
  ...
  <div class="mobile-drawer z-50 fixed top-0 right-0 h-full w-[280px] bg-white dark:bg-gray-900 transform translate-x-full transition-transform duration-300" id="mobileDrawer">
    ...
    <div class="mb-4 px-5">
      ...
      <select id="mobile-course-switcher" class="w-full p-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm" onchange="Student.switchCourseFromMobile(this.value)">
        ${AppState.joinedGroups && AppState.joinedGroups.length > 0 ?
...
export const Router = {
  initStudent: async () => {
    ...
    renderPage('dashboard', (contentEl) => {
      ...
      getStudentModule().then(Student => Student.loadDashboard());
    });

    const Student = await getStudentModule();
    ...
    setTimeout(() => {
      if (navigator.onLine && AppState.activeGroupId) {
        import('../../shared/services/offline-sync.js').then(({ OfflineSync }) => {
  ...
  student: async (p) => {
    ...
    // Access control: profile incomplete redirect
    ...
    // Access control: no teacher codes
    ...
    // Access control: no active group for most features except exceptions
    ...
    if (!exceptions.includes(p) && !AppState.activeGroupId) {
      Swal.fire({
        ...
        getStudentModule().then(Student => Student.showGroupCodeModal());
      });
      return;
    }
    ...
    const pageConfig = {
      profile: () => {
        renderPage('profile', (contentEl) => {
          contentEl.innerHTML = `<div class="p-10 text-center"><div class="quick-loader mx-auto"></div></div>`;
          getStudentModule().then(Student => Student.profile());
        });
        window.history.pushState({ route: 'profile' }, '');
      },
      dashboard: () => {
      ...
      courses: () => {
        renderPage('courses', (contentEl) => {
          contentEl.innerHTML = `<div class="p-10 text-center"><div class="quick-loader mx-auto"></div></div>`;
          getStudentModule().then(Student => Student.loadCourses());
        });
      ...
      rank: () => {
        renderPage('rank', (contentEl) => {
          contentEl.innerHTML = `<div class="p-10 text-center"><div class="quick-loader mx-auto"></div></div>`;
          getStudentModule().then(Student => Student.loadRankings());
        });
      ...
      results: () => {
        renderPage('results', (contentEl) => {
          contentEl.innerHTML = `<div class="p-10 text-center"><div class="quick-loader mx-auto"></div></div>`;
          getStudentModule().then(Student => Student.loadResults());
        });
      ...
      analysis: () => {
        renderPage('analysis', (contentEl) => {
          contentEl.innerHTML = `<div class="p-10 text-center"><div class="quick-loader mx-auto"></div></div>`;
          getStudentModule().then(Student => Student.loadAnalysis());
        });
      ...
      notices: () => {
        renderPage('notices', (contentEl) => {
          contentEl.innerHTML = `<div class="p-10 text-center"><div class="quick-loader mx-auto"></div></div>`;
          getStudentModule().then(Student => Student.loadNotices());
        });
      ...
      management: () => {
        renderPage('management', (contentEl) => {
          contentEl.innerHTML = `<div class="p-10 text-center"><div class="quick-loader mx-auto"></div></div>`;
          getStudentModule().then(Student => Student.loadManagement());
        });
